...
describe('computeServerTrustScore', () => {
  it('returns ~100 for a perfect mint (100% on every component)', () => {
    // uptime 100→45, nutCount 14→30, version 0.16→15, contact 3→5, audit errRate 0→5
    expect(computeServerTrustScore(100, 14, '0.16', 3, 100, 50, 0)).toBe(100)
  })
  ...
  it('computes from remaining components when audit data is missing', () => {
    ...
    expect(computeServerTrustScore(100, 14, '0.16', 3, null, null, null)).toBe(98)
  })
  ...
  describe('NUT support component (30%)', () => {
    ...
    it('contributes 30 at 14 nuts', () => {
      // 0 + 30 + 2.5 = 32.5 → 33
      expect(computeServerTrustScore(0, 14, null, 0, null, null, null)).toBe(33)
    })
    it('caps NUT support at 14 nuts (28 nuts gives the same score)', () => {
      expect(computeServerTrustScore(0, 28, null, 0, null, null, null)).toBe(33)
    })
  })
