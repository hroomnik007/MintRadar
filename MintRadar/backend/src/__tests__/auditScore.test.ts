...
describe('auditReliabilityScore', () => {
  it('returns 12.5 when there is no audit data at all (recentTotal null)', () => {
    expect(auditReliabilityScore(null, null)).toBe(12.5)
  })

  it('returns 12.5 (Unknown) below the minimum sample size, even with a bad error rate', () => {
    expect(auditReliabilityScore(0, 0)).toBe(12.5)
    expect(auditReliabilityScore(1, 1)).toBe(12.5)
    expect(auditReliabilityScore(AUDIT_MIN_SAMPLES - 1, AUDIT_MIN_SAMPLES - 1)).toBe(12.5)
  })

  it('scores normally once the minimum sample size is reached', () => {
    expect(auditReliabilityScore(AUDIT_MIN_SAMPLES, 0)).toBe(25)
  })

  it('gives 25 for a zero error rate', () => {
    expect(auditReliabilityScore(150, 0)).toBe(25)
  })

  it('gives 20 for error rate < 1%', () => {
    expect(auditReliabilityScore(1000, 5)).toBe(20) // 0.5%
  })

  it('gives 15 for error rate < 5%', () => {
    expect(auditReliabilityScore(100, 3)).toBe(15) // 3%
  })

  it('gives 10 for error rate < 15%', () => {
    expect(auditReliabilityScore(100, 10)).toBe(10) // 10%
  })

  it('gives 5 for error rate >= 15%', () => {
    expect(auditReliabilityScore(20, 10)).toBe(5) // 50%
  })

  it('treats null recentErrors as zero errors', () => {
    expect(auditReliabilityScore(100, null)).toBe(25)
  })
  ...
  it('does not penalize a mint whose recent swaps are clean, regardless of a troubled past', () => {
    ...
    expect(auditReliabilityScore(100, 0)).toBe(25)
  })
})
