...
// GET /api/stats/reliability-movers — same mocking approach as mints-known.test.ts /
...
// mocked rows represent what the query over `mints` returns — last_reliability_score
// as latest_score and the rolled-up reliability_score_{7,30}d_ago as old_score. A
...
describe('GET /api/stats/reliability-movers', () => {
  it('returns risers and fallers computed from the resolved snapshot rows', async () => {
    ...
    const res = await request(app).get('/api/stats/reliability-movers')
  ...
  it('accepts period=30d and requests a 30-day cutoff', async () => {
    query.mockResolvedValueOnce({ rows: [] })
    const res = await request(app).get('/api/stats/reliability-movers').query({ period: '30d' })
  ...
  it('falls back to 7d for an invalid period value instead of erroring', async () => {
    query.mockResolvedValueOnce({ rows: [] })
    const res = await request(app).get('/api/stats/reliability-movers').query({ period: 'bogus' })
    expect(res.status).toBe(200)
    expect(res.body.period).toBe('7d')
  })
  ...
  it('serves the second request for the same period from cache without querying again', async () => {
    ...
    const first = await request(app).get('/api/stats/reliability-movers')
    const second = await request(app).get('/api/stats/reliability-movers')
