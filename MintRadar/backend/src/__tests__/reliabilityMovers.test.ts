...
import { computeReliabilityMovers, RELIABILITY_MOVER_THRESHOLD, type MintScoreSnapshot } from '../reliabilityMovers.js'
...
describe('computeReliabilityMovers', () => {
  ...
  describe('no qualifying mints', () => {
    it('returns empty risers and fallers when every delta is within +/-3', () => {
      const result = computeReliabilityMovers([snap('a', 81, 80), snap('b', 79, 80), snap('c', 80, 80)])
      expect(result).toEqual({ risers: [], fallers: [] })
    })

    it('returns empty arrays for an empty snapshot list', () => {
      expect(computeReliabilityMovers([])).toEqual({ risers: [], fallers: [] })
    })
  })
  ...
  describe('ranking and top-3 cap', () => {
    ...
    it('sorts fallers by delta ascending (most negative first) and caps at 3', () => {
      const { fallers } = computeReliabilityMovers([
        snap('small', 76, 80),        // -4
  ...
  it('a mint excluded for insufficient history simply never appears in the input snapshots (SQL INNER JOIN concern, not this function)', () => {
    ...
    // computeReliabilityMovers only ever sees mints that already have both snapshots.
    const result = computeReliabilityMovers([snap('has-both-snapshots', 90, 80)])
    expect(result.risers).toHaveLength(1)
  })

  it('preserves mint name and url in the output', () => {
    const { risers } = computeReliabilityMovers([snap('https://mint.example.com', 90, 80, 'Example Mint')])
  ...
  it('handles a null mint name without crashing', () => {
    const { risers } = computeReliabilityMovers([snap('https://mint.example.com', 90, 80, null)])
